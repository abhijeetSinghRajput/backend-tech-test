...
// Query: *[_type == "blog" && (seoHideFromLists != true) && (!defined($category) || $category in categories[]->slug.current)] | order(orderRank asc) [$start...$end]{      _type,  _id,  title,  description,  "slug":slug.current,  orderRank,    image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  },  ogImage,  publishedAt,  "readTime": round(length(pt::text(richText)) / 5 / 200),    authors[0]->{    _id,    name,    position,      image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  }  },    categories[]->{    _id,    title,    "slug": slug.current  },  pokemon {    name,    id,    image  }  }
...
// Query: count(*[_type == "blog" && (seoHideFromLists != true) && (!defined($category) || $category in categories[]->slug.current)])
