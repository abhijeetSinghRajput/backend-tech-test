...
import { Joint, VerticalLine } from "./elements/grid-ui";
...
function NavbarSkeleton() {
  return (
    <header className="relative z-40 w-full border-b bg-background/80 backdrop-blur-sm">
...
export function Navbar({
  ...
  const { logo: _logo, siteTitle: _siteTitle } = settingsData || {};
  ...
  return (
    <header className="relative z-40 w-full border-b border-border bg-background/80 backdrop-blur-sm">
      <div className="container mx-auto px-4 md:px-8 max-w-[1400px] relative">
        ...
        <div className="flex h-16 items-center justify-between">
          {/* Logo container with joint lines */}
          <div className="flex h-16 w-48 items-center border-x border-border/50 px-4 relative">
          ...
          {/* Desktop Actions with joint lines */}
          <div className="hidden h-16 items-center gap-4 md:flex border-x border-border/50 px-6 relative">
            <ModeToggle />
            <SanityButtons
              buttonClassName="rounded-none border-border"
              buttons={buttons || []}
