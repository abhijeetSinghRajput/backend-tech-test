...
export const queryBlogIndexPageBlogs = defineQuery(`
  *[_type == "blog" && (seoHideFromLists != true) && (!defined($category) || $category in categories[]->slug.current)] | order(orderRank asc) [$start...$end]{
    ${blogCardFragment}
  }
`);
...
export const queryBlogIndexPageBlogsCount = defineQuery(`
  count(*[_type == "blog" && (seoHideFromLists != true) && (!defined($category) || $category in categories[]->slug.current)])
`);
