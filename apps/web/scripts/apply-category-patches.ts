...
const keywordMap: { keywords: string[]; cat: string }[] = [
...
async function applyPatches() {
  ...
  const blogs: Blog[] = await client.fetch(`*[_type == "blog"] { _id, title }`);
  ...
  blogs.forEach((blog: Blog) => {
    ...
    keywordMap.forEach((item: { keywords: string[]; cat: string }) => {
      if (item.keywords.some((k: string) => title.includes(k))) {
        const catId = categories[item.cat];
        if (catId) assignedCats.add(catId);
      }
    });
